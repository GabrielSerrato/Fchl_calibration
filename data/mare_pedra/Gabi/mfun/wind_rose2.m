function varargout = wind_rose2(D,F,tamanho_total,varargin)
...
load('TCC/programas/mfun/jet_claro2.mat')
%figure;
handles=[];
...
for i=1:length(Ay)-1
  if n(i)
    ...
    for j=1:length(Ag)-1
      ...
      y=[r1*cos(t(1)) r2*cos(t) r1*cos(fliplr(t))];
      x=[r1*sin(t(1)) r2*sin(t) r1*sin(fliplr(t))];  % Barlaman Modification here (Watch OUT!!)
...
h(1)=text(-g-ri, 0,'OESTE', 'VerticalAlignment','top',   'HorizontalAlignment','left', args{:});
h(2)=text( g+ri, 0,'LESTE', 'VerticalAlignment','top',   'HorizontalAlignment','right',args{:});
h(3)=text( 0,-g-ri,'SUL','VerticalAlignment','bottom','HorizontalAlignment','left', args{:});
h(4)=text( 0, g+ri,'NORTE','VerticalAlignment','top',   'HorizontalAlignment','left', args{:});
handles=[handles h];
...
elseif legType==2 % separated boxes.
  for j=1:length(Ag)-1
    ...
    handles(end+1)=text(x1+L/4,(y0+dy+y1)/2,lab,'VerticalAlignment','middle','fontsize',12);
    y0=y1;
  end
...
x=mean([-g*rs,g*rl]);
y=mean([g+ri,g*rs]);
handles(end+1)=text(x,y,titStr,'HorizontalAlignment','center','fontweight','bold');

x=x0;
y=y1+dy;
handles(end+1)=text(x,y,legStr,'HorizontalAlignment','left','VerticalAlignment','bottom','fontsize',12);
