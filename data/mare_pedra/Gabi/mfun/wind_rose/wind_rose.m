...
% wind_rose(dados(extremos,3),dados(extremos,1),'n',16,'di',[10 12 15 18 20 35],'quad',2,'cmap',ans)
...
for i=1:ncircles
  ...
  labs(i)=text((ci(i)+ri)*cos(Ang(quad)),(ci(i)+ri)*sin(Ang(quad)),[num2str(ci(i)),'%'],...
      ...
      'BackgroundColor',percBg,'FontSize',9);
end
...
% N S E W labels:
bg='none';
args={'BackgroundColor',bg,'FontSize',11};
...
if legType==1 % contimuous.
  for j=1:length(Ag)-1
    ...
    handles(end+1)=text(x1+L/4,y0,lab,'VerticalAlignment','middle','fontsize',9);
    y0=y1;
  end
  if ~ (hasH & IncHiLow)
    handles(end+1)=text(x1+L/4,y0,num2str(Ag(end)),'VerticalAlignment','middle','fontsize',9);
  end
elseif legType==2 % separated boxes.
  for j=1:length(Ag)-1
    ...
    handles(end+1)=text(x1+L/4,(y0+dy+y1)/2,lab,'VerticalAlignment','middle','fontsize',9);
    y0=y1;
  end
...
x=x0;
y=y1+dy;
handles(end+1)=text(x,y,legStr,'HorizontalAlignment','left','VerticalAlignment','bottom','fontsize',9);
